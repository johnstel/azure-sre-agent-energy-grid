...
function mergeEvidencePackage(existing: RehearsalEvidencePackage, updates: Partial<RehearsalEvidencePackage>): RehearsalEvidencePackage {
  const attachmentChecksums = { ...existing.attachmentChecksums, ...updates.attachmentChecksums };
  return {
    ...existing,
    ...updates,
    attachmentChecksums,
    redactionFindings: [...(updates.redactionFindings ?? existing.redactionFindings ?? [])],
    sensitivePatterns: [...(updates.sensitivePatterns ?? existing.sensitivePatterns ?? [])],
  };
}
...
async function ensureEvidenceFile(candidatePath: string): Promise<void> {
  ...
  try {
    ...
    await writeFile(normalizedPath, 'Mission Control rehearsal placeholder evidence.\n', 'utf8');
  }
}
...
export async function updateRehearsalEvidence(request: UpdateRehearsalEvidenceRequest): Promise<RehearsalRun> {
  ...
  const checksumEntries = Object.entries(request.attachmentChecksums ?? {});
  for (const [label, expectedChecksum] of checksumEntries) {
    if (label && request.evidencePath && expectedChecksum) {
      const actualChecksum = await computeFileChecksum(request.evidencePath);
      if (actualChecksum !== expectedChecksum) {
        nextPackage.redactionFindings = Array.from(new Set([...nextPackage.redactionFindings, `Checksum mismatch for ${label}`]));
      }
    }
  }
