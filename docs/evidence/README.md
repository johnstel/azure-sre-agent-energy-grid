...
- **Screenshots**: `{scenario-id}_{step}.png` — e.g., `oom-killed_diagnosis.png`. PNG format, max 1920×1080. Redact secrets before committing.
...
| T3 — Diagnosis received | | Agent returns root cause |
| T4 — Remediation applied | | Human approves / executes fix |
...
| Screenshot: SRE Agent diagnosis | ☐ | `screenshots/{scenario}_diagnosis.png` | |
| Screenshot: action proposal | ☐ | `screenshots/{scenario}_proposal.png` | |
| Screenshot: recovery state | ☐ | `screenshots/{scenario}_recovered.png` | |
...
2. **1 diagnosis screenshot** — SRE Agent portal showing root cause identification
...
- Naming: `{scenario-id}_{step}.png` (e.g., `oom-killed_diagnosis.png`)
