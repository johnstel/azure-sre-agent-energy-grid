...
| **Key Vault** | Soft delete enabled; purge protection **enabled by default** and any disposable-demo opt-out is an explicit pre-deployment choice rather than a later reversible toggle | 7-day soft delete + retained deleted vault names while purge protection remains enabled | `key-vault.bicep:42-49` |
