...
This document prevents accidental overclaiming of Preview capabilities. Review it before every demo.
...
| **Auto-Remediation** | "SRE Agent automatically fixes issues" | "SRE Agent proposes fixes in Review mode. The operator approves before any action executes. Auto mode exists but is not demonstrated." | The demo runs `mode: 'Review'`. Auto mode is out of scope and requires a separate security review. |
| **Audit Trail** | "Full audit trail of every agent action" | "SRE Agent operational telemetry is configured to App Insights; ARM-level actions appear in the Activity Log. Exact conversation/action fields are SCHEMA_TBD until verified in the deployed Preview service. We capture evidence per demo run." | Full audit trail requires Activity Log export to Log Analytics (not yet configured) and SRE Agent conversation retention (Preview — opaque). |
...
| **Data Retention** | "Complete audit history" | "Log Analytics retains 30 days; App Insights retains 90 days. Activity Log is not yet exported to Log Analytics for KQL querying. See our retention table in CAPABILITY-CONTRACTS.md §11." | 30-day LA vs 90-day AI creates a 60-day evidence gap. Activity Log export is a Wave 1 item. |
| **Preview Status** | "Production-ready" or "GA" | "Azure SRE Agent is in Public Preview. The API version is `2025-05-01-preview`. Telemetry schemas may change." | Must appear in every customer-facing artifact, not just footers. |
...
## Preview Disclosure Requirements

The following statement (or equivalent) must appear **prominently** in every customer-facing artifact:

> **Azure SRE Agent is in Public Preview.** Features, APIs, telemetry schemas, and pricing may change before General Availability. This demo lab uses the `2025-05-01-preview` API version.
