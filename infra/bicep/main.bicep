...
@description('Enable Azure Key Vault purge protection for the lab vault. Defaults to true for secure-by-default deploys. Set to false only before deployment for an explicitly disposable demo lab where rapid delete/recreate and same-name reuse are intentionally accepted. Azure rejects an explicit false value; once enabled, it cannot be turned off and deleted vault names remain retained for the retention period.')
